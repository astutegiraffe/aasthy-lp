...
function getRelativePos(container, left) {
	let width = parseInt(window.getComputedStyle(document.querySelector('.asset-investment-card-lp')).width);
...
function loadingEvent() {
	...
	let curEl = document.getElementById(`shared-investments-lp-assets`);
	document.getElementById('shared-investments-lp-next-btn').addEventListener('click', () => {
		scrollToElm(curEl, 0.4, 1);
	});

	document.getElementById('shared-investments-lp-prev-btn').addEventListener('click', () => {
		scrollToElm(curEl, 0.4, -1);
	});
}
